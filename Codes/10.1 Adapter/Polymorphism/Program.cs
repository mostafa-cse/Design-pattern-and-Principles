    ...
    class Program
    {
        ...
        public sealed class clsDepartmentDB
        {
            ...
            public clsDepartment getDepartment(int nDepartmentID)
            {
                ...
                // Not allowed when abstract. At that time we need a Static Method
            }
        }
        ...
        public class clsCollegeTeacherHRSystem
        {
            public string[][] fetchCollegeTeachers()
            {
                string[][] sCollegeTeachers = new string[2][];

                sCollegeTeachers[0] = new string[] { "BL College", "Afroza Sultana XYZ", "01/01/1979", "Jashore", "Assistant Professor", "CSE" };
                sCollegeTeachers[1] = new string[] { "MM College", "Moni Khan", "01/01/1982", "Khulna", "Lecturer", "EEE" };

                return sCollegeTeachers;
            }
        }
        ...
        {
            public string[][] fetchOfficers()
            {
                string[][] sEmployees = new string[2][];

                sEmployees[0] = new string[] { "Reza 1234", "01/01/1979", "Jashore", "Clark (Grade-2)", "HR" };
        ...
        {
            private clsCollegeTeacherHRSystem _oCollegeTeacherHRSystem;

            public clsTeacherAdapter(clsCollegeTeacherHRSystem oCollegeTeacherHRSystem)
            {
                _oCollegeTeacherHRSystem = oCollegeTeacherHRSystem;
            }
            ...
            public List<clsEmployee> getEmployees()
            {
                ...
                string[][] sCollegeTeachers = _oCollegeTeacherHRSystem.fetchCollegeTeachers();

                foreach (string[] sEmployee in sCollegeTeachers)
                {
                    clsFacultyFactory oFacultyFactory = new clsFacultyFactory();

                    oEmployees.Add(oFacultyFactory.getTeacher("MSc", sEmployee[1], sEmployee[2], sEmployee[3], sEmployee[4], sEmployee[5]));
                }

                clsLegacyTeachingHRSystem oLegacyTeachingHRSystem = new clsLegacyTeachingHRSystem();

                string[][] sEmployees = oLegacyTeachingHRSystem.fetchTeachers();
        ...
        public class clsOfficerAdapter : IEmployeeAdapter
        {
            public List<clsEmployee> getEmployees()
            {
                ...
                string[][] sEmployees = oLegacyOfficialHRSystem.fetchOfficers();
        ...
        static void Main(string[] args)
        {
            ...
            List<IEmployeeAdapter> oAdapters = new List<IEmployeeAdapter>();

            clsTeacherAdapter oAdapterT = new clsTeacherAdapter(new clsCollegeTeacherHRSystem());

            oAdapters.Add(oAdapterT);

            clsOfficerAdapter oAdapterO = new clsOfficerAdapter();

            oAdapters.Add(oAdapterO);

            Console.WriteLine("######### List of Employees ##########");

            foreach (IEmployeeAdapter oAdapterALL in oAdapters)
                foreach (clsEmployee oEmployee in oAdapterALL.getEmployees())
                {
                    oEmployee.WriteInfo();
                }
